...
def colbert_score_reduce(scores_padded, D_mask):
    ...
    # add: scores_padded.shape: [n_docs, doc_dims(cls + doc), query_dims(text + cls + patch)]
    scores_padded[:, :32, 64: ] = -9999
...
def _sort_by_length(ids, mask, bsize, *args):
    ...
    # modified
    if ids.size(0) <= bsize:
        return_array = [ids, mask]
        for arg in args:
            return_array.append(arg)
        return *return_array, torch.arange(ids.size(0))
