...
def find_all_linear_names(model):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, cls):
            ...
            # modified: accurate name matching, since vit also have linears
            lora_module_names.add('.'.join(names))

    # backup
    # if "lm_head" in lora_module_names:  # needed for 16-bit
    #     lora_module_names.remove("lm_head")
    # modified: accurate name matching
...
def train():
    ...
    # add: add custom finetuning dataset
    from llava.data.datasets_mixture import add_dataset
    from llava.data.datasets_mixture import Dataset as ViLADatasetInfo
    if not data_args.data_mixture:
        print(f"## using `data_path` and `image_folder` to create dataset...")
        sft_dataset = ViLADatasetInfo(
            dataset_name="__sft_dataset",
            dataset_type="torch",
            data_path=data_args.data_path,
            image_path=data_args.image_folder
        )
        add_dataset(sft_dataset)
        data_args.data_mixture = "__sft_dataset"
    ...
    # add: print trainables and not_trainables
    if training_args.local_rank == 0:
        from pprint import pformat
        trainables, not_trainables = [], []
        n_trainables, n_not_trainables = 0, 0
        for pn, p in model.named_parameters():
            if p.requires_grad:
                trainables.append(pn)
                n_trainables += p.numel()
            else:
                not_trainables.append(pn)
                n_not_trainables += p.numel()
        print(
            f"## trainables: {pformat(trainables)}",
            f"## not trainables: {pformat(not_trainables)}",
            f"## trainables params: {n_trainables:,}, total: {n_trainables + n_not_trainables:,}",
            sep='\n'
        )
