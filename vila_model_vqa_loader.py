...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(image_file, list):
            assert len(image_file) == qs.count(DEFAULT_IMAGE_TOKEN), f'In vila eval, if `image` field is a list, make sure there are same amount of {DEFAULT_IMAGE_TOKEN} in text.'
            
            images = [Image.open(os.path.join(self.image_folder, rpath)).convert('RGB') for rpath in image_file]
            image_tensor = process_images(images, self.image_processor, self.model_config)
        else:
            assert DEFAULT_IMAGE_TOKEN not in qs, f'In llava eval, if `image` field is a str, make sure there are NO {DEFAULT_IMAGE_TOKEN} in text.'
