...
def pprint(*args, **kwargs):
    print(pformat(*args, **kwargs), flush=True)
...
def query_index(args, ds, passage_contents, passage_ids, flmr_model: FLMRModelForRetrieval):
        ...
        for i, (question_id, pos_ids) in enumerate(zip(batch["question_id"], batch["pos_item_ids"])):
            ...
            if args.compute_pseudo_recall:
                ...
                for retrieved_doc_text in retrieved_doc_texts:
                    # add
                    if not isinstance(retrieved_doc_text, str):
                        retrieved_doc_text = retrieved_doc_text[0]
    ...
    pprint("## encode_and_search_batch")
    ds = ds.map(
...
def main(args):
    ...
    if args.sample_examples is not None:
        print(f"## selecting {args.sample_examples} examples.")
        ds = ds.shuffle(seed=42).select(range(args.sample_examples))
    ...
    pprint("## add_path_prefix_in_img_path")
    ...
    if not args.index_use_images:
        print('## using text only on document side.', flush=True)
        passage_contents = passage_ds["passage_content"]
        # passage_contents =['<BOK> ' + passage + ' <EOK>' for passage in passage_contents]
    else:
        assert args.doc_image_root_dir
        assert args.doc_image_title2image
        print('## using text & image on document side.', flush=True)
        doc_image_title2image = json.load(open(args.doc_image_title2image, 'r', encoding='utf-8'))
        
        def add_passage_img_path(example, doc_image_root_dir, doc_image_title2image, title_key):
            example["passage_img_path"] = os.path.join(doc_image_root_dir, doc_image_title2image[example[title_key]])
            return example
        
        print('## add_passage_img_path...', flush=True)
        passage_ds = passage_ds.map(add_passage_img_path, 
                                    fn_kwargs={"doc_image_root_dir": args.doc_image_root_dir,
    ...
    pprint("## prepare_inputs")
    ds = ds.map(prepare_inputs, keep_in_memory=True)
    ...
    # add
    dict_to_record = list(dict_to_record.items())
...
if __name__ == "__main__":
    ...
    # add
    parser.add_argument("--index_use_images", type=distutils.util.strtobool, default=False)
    parser.add_argument("--doc_image_root_dir", type=str, default='')
    parser.add_argument("--doc_image_title2image", type=str, default='')
    parser.add_argument("--sample_examples", type=int, default=None)
    parser.add_argument("--title_key", type=str, default='title')
